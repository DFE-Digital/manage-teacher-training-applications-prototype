...
exports.getSubjects = (text) => {
  let data = SUBJECTS

  if (text) {
    data = data.filter(subject => subject.text.toLowerCase().includes(text.toLowerCase()))
  }

  return data
}

exports.getQualifications = (type) => {
  let data = QUALIFICATIONS

  if (type === 'undergraduate') {
    data = data.filter(qualification => qualification.level === 6 || qualification.level === 7)
  }

  return data
}

exports.getInstitutions = (text) => {
  let data = INSTITUTIONS

  if (text) {
    data = data.filter(institution => institution.toLowerCase().includes(text.toLowerCase()))
  }
